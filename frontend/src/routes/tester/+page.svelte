<script lang="ts">
	...
	const setupDeviceWsConnection = async (device: EegDevice) => {
		device.ws = new WebSocket(`ws://localhost:8080`)
		device.ws.onerror = () => {
			alert(TEXTS.EXCEPTON_BACKEND_CRUSH)
		}
			...
			<div>
				{#await typeof qrcode !== 'undefined' ? qrcode.toString(`http://${ip}:3000/testee#${device.deviceId}`) : '' then image}
					<HoverCard.Root>
						<HoverCard.Trigger>
							<button
								class="h-10 w-10 scale-150 rounded-full bg-gray-200 hover:bg-gray-400"
								onclick={() => {
...
{#snippet ToolBar()}
	<div class="flex w-full flex-col gap-3 border-t border-gray-200 px-5 pt-5">
		...
		<Button
			disabled={enableUploading}
			class="w-full flex-1 cursor-pointer bg-orange-500 py-3 font-black text-gray-100 transition-all duration-150 hover:scale-99 hover:bg-orange-600"
			onclick={() => {
				const { ws } = eegDevices[0]
				/** @note 默认导出全部 */
				ws!.send(JSON.stringify(['export_csv', null]))
				alert(TEXTS.PLEASE_CHECK_CSV)
			}}
		>
			{TEXTS.DATABASE_TO_TABLE}
		</Button>
		<Button
			class={`w-full flex-1 cursor-pointer py-3 font-black text-gray-100 transition-all duration-150 hover:scale-99 ${
				enableUploading ? 'bg-red-500 hover:bg-red-600' : 'bg-lime-600 hover:bg-lime-700'
			}`}
			onclick={() => {
